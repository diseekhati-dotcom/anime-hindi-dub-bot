...
# RareToon/RareAnimes has used multiple domains. Keep the known current
# domains as fallbacks so one moved domain does not break searching.
RARE_BASES = [
    "https://www.rareanimes.mov/",
    "https://rareanimes.asia/",
    "https://raretoonsindia.in/",
]
...
class RareAnimesSource:
    ...
    def search(self, query: str, limit: int = 12) -> List[Tuple[str, str]]:
        ...
        # Try all known RareToon/RareAnimes domains. A domain can be live
        # while its old search endpoint is unavailable.
        for base in RARE_BASES:
            urls = [
                urljoin(base, f"?s={q}"),
                urljoin(base, f"search/{q}/"),
            ]
            ...
            for search_url in urls:
                ...
                for url, text in collect_links(soup, search_url):
                    candidate = text or url.split("/")[-1].replace("-", " ")
                    score = title_similarity(query, candidate)

                    # Also accept exact query tokens in the URL/title. This
                    # helps when the site uses long Hindi-dubbed slugs.
                    nq = normalize_title(query)
                    nc = normalize_title(candidate)
                    token_hit = bool(nq and nc and all(
                        token in nc for token in nq.split()
                    ))

                    if score >= 0.30 or token_hit:
                        found.append((url, text))

            # Don't stop after the first domain: newer domains may contain
            # results missing from the older mirror.
...
class AnimeMirchiSource:
    ...
    def parse_page(self, url: str, query: str = "") -> Optional[AnimeInfo]:
        ...
        if query:
            ...
            # Long site titles often contain "Hindi Dubbed Episodes" etc.
            # Compare against the cleaned title and the slug as a fallback.
            slug_title = urlparse(url).path.rstrip("/").split("/")[-1].replace("-", " ")
            slug_score = title_similarity(query, slug_title)
            if max(score, slug_score) < 0.24:
                return None
            info.confidence = max(score, slug_score)
...
class AnimeScraper:
    ...
    def scrape_url(self, url: str) -> Optional[AnimeInfo]:
        ...
        if (
            ("rareanimes" in host or "raretoonsindia" in host)
            and self.rare
        ):
