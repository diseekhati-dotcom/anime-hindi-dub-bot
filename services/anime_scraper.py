...
class AnimeScraper:
    ...
    def search_anime(
        ...
        # -------------------------------------------------------------
        # Run the three independent data sources at the same time.
        # AnimeDubHindi remains the primary source for schedule,
        # episode, language, season and release-date information.
        # Anime Mirchi is used for platform / Dub By information.
        # Jikan/MAL is used for poster and studio fallback.
        # -------------------------------------------------------------

        def fetch_animedubhindi():
            scraper = AnimeScraper()
            return scraper._find_animedubhindi(
                anime_name,
                query
            )
        ...
        def fetch_mal():
            scraper = AnimeScraper()
            return scraper._get_mal_info(
                anime_name
            )

        def fetch_anime_mirchi():
            scraper = AnimeScraper()
            return scraper._search_anime_mirchi(
                anime_name,
                query
            )
        ...
        with ThreadPoolExecutor(
            ...
            futures = {
                ...
                executor.submit(fetch_anime_mirchi): "animemirchi",
            }
        ...
        # -------------------------------------------------------------
        # If neither AnimeDubHindi nor MAL knows the anime
        # -------------------------------------------------------------

        if not result and not mal and not mirchi:
        ...
        # -------------------------------------------------------------
        # Build fallback result from MAL / Anime Mirchi
        # -------------------------------------------------------------
        ...
        if not result:
            ...
            result = {
                ...
                "hindi_dub": (
                    mirchi.get("hindi_dub", "Not Verified")
                    if mirchi
                    else "Not Verified"
                ),
        ...
        # -------------------------------------------------------------
        # Merge MAL without replacing richer AnimeDubHindi data.
        # -------------------------------------------------------------
        ...
        if mal:
            ...
            # AnimeDubHindi remains the preferred source for studio.
            # MAL is only a fallback when AnimeDubHindi has no studio.
            if not result.get("studio"):
                result["studio"] = mal.get("studio")
        ...
        # -------------------------------------------------------------
        # Anime Mirchi is now the platform / Dub By source.
        # Do NOT search every OTT service individually.
        # -------------------------------------------------------------

        if mirchi:

            if mirchi.get("platform_entries"):
                result["platform_entries"] = (
                    self._dedupe_platform_entries(
                        ...
                        + mirchi["platform_entries"]
                    )
                )

            if mirchi.get("platform"):
                result["platform"] = mirchi["platform"]

            if mirchi.get("dub_by"):
                result["dub_by"] = mirchi["dub_by"]
            ...
            if (
                ...
                result["name"] = mirchi["name"]

        # -------------------------------------------------------------
        # Keep AnimeDubHindi as the source for schedule / time,
        # release date, season, episode and languages.
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # Movie protection.  A movie must never inherit MAL series
        # seasons or episode counts.
        # -------------------------------------------------------------

        combined_name = str(
            result.get("name", anime_name)
        )

        if self._is_movie_result(
            combined_name,
            result.get("hindi_details"),
        ):
            result["season"] = None
